              ...
              return (
                <li key={idx} className='nav-item dropdown megamenu'>
                  <Link
                    ...
                    style={{ fontSize: "18px" }}
                  >
                    <i className='fas fa-arrow-right text-light navbar-menu-icon-right'></i>{" "}
                  ...
                  <div aria-labelledby='megamneu' className='dropdown-menu'>
                    <div className='row rounded-0 m-0 w-100'>
                      <div className='col-lg-12'>
                        <div className='row'>
                          {categoryItems.subCategoryList.map(
                            (subCategoryItems, indx) => {
                              return (
                                <div key={indx} className='col-lg-3 mb-4'>
                                  <h6
                                    className='font-weight-bolder text-uppercase dropdown-toggle'
                                    style={{ fontSize: "13px" }}
                                  >
                                    <i className='fas fa-arrow-right text-light navbar-menu-icon-right'></i>{" "}
                                  ...
                                  <ul className='list-unstyled ml-4'>
                                    <hr
                                      className='navbar-hr my-1'
                                      style={{
                                        border: "solid 1px white",
                                      }}
                                    />
                                    {subCategoryItems.products.map(
                                      (item, index) => {
                                        return (
                                          <li
                                            key={index}
                                          ...
                                          >
                                            <Link
                                              to={`/browser/${item.productName}/${item.id}`}
                                              className='nav-link main-item p-0'
                                            >
                                              <i className='fas fa-arrow-right text-light navbar-menu-icon-right'></i>{" "}
                                              {item.productName}
                                            </Link>
